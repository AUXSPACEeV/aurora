...
/* Fallback cap for blocking ops whose table entry leaves .timeout_us at 0.
 * The op table treats 0 as "wait forever"; in a Zephyr device-init context that
 * lets a non-responding BNO08x (e.g. nINT never asserts) spin opProcess() and
 * hang the boot thread indefinitely. Bounding it turns that into a clean init
 * failure so the rest of the system still boots. */
#define SH2_OP_DEFAULT_TIMEOUT_US (3000000)
...
static int opProcess(sh2_t* pSh2, const sh2_Op_t* pOp) {
    ...
    /* A 0 in the op table means "no timeout"; bound it so a silent sensor can
     * never hang the boot thread (see SH2_OP_DEFAULT_TIMEOUT_US). */
    uint32_t timeout_us =
        (pOp->timeout_us != 0) ? pOp->timeout_us : SH2_OP_DEFAULT_TIMEOUT_US;
