...
off the SD card (or the whole `DATA` directory) and page through the
flight's graphs without running a command line.
...
- **Reads the binary flight log directly.** The data logger's `bin`
  ...
  readable.
- **Browses a whole `DATA` directory.** Point it at the card's data
  logger directory and every log on it is listed with its size:
...
- **Finds the flight window.** Binary logs record sensor data only, so
  there is no `ARMED → BOOST` transition in the file. A `state_audit`
  file sitting next to the log is used when present; otherwise the
  boost and touchdown instants are recovered from the measurements
  using the same heuristics the CLI falls back to, and the plot title
  says which signal they came from.
...
- `PATH` — a flight log (`.bin`, `.csv`, `.influx`) or a `DATA`
  directory to open on start-up. Optional; the window's **Open Log…**
  and **Open Folder…** buttons do the same thing.
...
| Area | What it holds |
|---|---|
| **Flight logs** | Tree of the opened folder. Selecting a file decodes it and hangs its flights underneath. |
