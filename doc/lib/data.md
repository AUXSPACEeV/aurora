...
| Command | Description |
| --- | --- |
| `data_logger list` | List every registered logger with its formatter and current state (`running`, `stopped` or `closed`). |
| `data_logger start <name>` | Start a logger. Writes the formatter header if applicable. |
| `data_logger stop <name>` | Stop a logger. Subsequent writes are dropped until restarted. |
| `data_logger status <name>` | Show the state of a single logger. |
| `data_logger flush <name>` | Flush buffered data to the backing storage. |
