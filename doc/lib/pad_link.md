...
ERROR`. This ordinal table is append-only (see the decoder warning
in `tools/rec_zephyr.py`), so existing values never move.
...
A runnable Python reference using [Bleak](https://bleak.readthedocs.io)
