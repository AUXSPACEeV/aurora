...
**Source:** [`flight_computer`](https://github.com/AUXSPACEeV/aurora/tree/main/flight_computer)

The flight computer application is a simple flight computer for model rocketry.

AURORA is built to run on multiple interconnected PCBs that communicate over CAN
bus, i2c or other fieldbus technologies.
`flight_computer` manages the IMU and barometric sensor data, uses the flight state
...
`flight_computer` uses up to three additional Zephyr threads
...
`native_sim` board target), `flight_computer` replaces the real IMU and baro
...
Any board that wants to run `flight_computer` has to provide the following
