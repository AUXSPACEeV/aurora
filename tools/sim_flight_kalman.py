...
def main():
    ...
    # Top: raw pressure
    ax_press.plot(t, noisy_press / 100, color="#bbbbbb", linewidth=0.5,
                  label="Noisy baro reading")
    ax_press.plot(t, true_press / 100, color="#9141ac", linewidth=1.5,
                  label="True pressure")
    if apogee_idx is not None:
        ax_press.axvline(t[apogee_idx], color="#c01c28", linestyle="--", linewidth=1.5)
    ...
    # Middle: altitude
    ax_alt.plot(t, baro_alt, color="#bbbbbb", linewidth=0.5,
                label="Baro altitude (hypsometric)")
    ax_alt.plot(t, true_alt, color="#1a5fb4", linewidth=2,
                label="True altitude")
    ax_alt.plot(t, filtered_alt, color="#e5a50a", linewidth=2,
                label="Kalman filter output")
    if apogee_idx is not None:
        ax_alt.axvline(t[apogee_idx], color="#c01c28", linestyle="--", linewidth=1.5,
                       label=f"Apogee detected (t={t[apogee_idx]:.2f} s)")
        ax_alt.plot(t[apogee_idx], filtered_alt[apogee_idx], "o", color="#c01c28", markersize=8)
    ax_alt.set_ylabel("Altitude (m)")
    ax_alt.legend(loc="upper right")
    ax_alt.grid(True, alpha=0.3)

    # Bottom: velocity
    ax_vel.plot(t, filtered_vel, color="#26a269", linewidth=1.5,
                label="Estimated velocity")
    ax_vel.axhline(0, color="black", linewidth=0.5)
    if apogee_idx is not None:
        ax_vel.axvline(t[apogee_idx], color="#c01c28", linestyle="--", linewidth=1.5)
