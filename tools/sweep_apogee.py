...
grid = list(itertools.product(
    ...
    [0.5, 0.6, 0.7, 0.8, 1.0],    # delta_h
    [1, 2],                       # debounce
))

print(f"Sweeping {len(grid)} configs")
results = []
for q_vel, r_meas, delta_h, debounce in grid:
    deltas = []
    valid = True
    ...
    results.append((q_vel, r_meas, delta_h, debounce, f1, f2, worst, valid))

valid_results = [r for r in results if r[7]]
valid_results.sort(key=lambda r: r[6])
print(f"\nTop 15 configs (lowest worst-case lag, no early fires):")
print(f"{'q_vel':>6} {'r':>5} {'Δh':>5} {'db':>3} "
      f"{'f1_Δ':>7} {'f2_Δ':>7} {'worst':>7}")
for q_vel, r_meas, delta_h, debounce, f1, f2, worst, _ in valid_results[:15]:
    print(f"{q_vel:>6.1f} {r_meas:>5.1f} {delta_h:>5.2f} {debounce:>3d} "
